...
/-!
# Limits of approximations on expanding neighbourhoods

The approximating functions need only eventually be holomorphic near each point.
This extends the entire-stage argument first used in the application's
`EremenkosConjecture/EntireLimit.lean` to the neighbourhood stages of Arakelian's
proof.
-/
...
theorem exists_entire_limit_of_eventually_holomorphic_corrections
    (F : ℕ → ℂ → ℂ)
    (hhol : ∀ z : ℂ, ∃ V : Set ℂ, IsOpen V ∧ z ∈ V ∧
      ∀ᶠ n : ℕ in atTop, DifferentiableOn ℂ (F n) V)
  ...
  intro z
  obtain ⟨V, hV, hzV, hFV⟩ := hhol z
  exact (hconv.tendstoLocallyUniformlyOn.differentiableOn hFV hV).differentiableAt
    (hV.mem_nhds hzV)
