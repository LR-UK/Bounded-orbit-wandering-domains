...
/-- One polynomial Runge approximation can retain finitely many return
...
theorem polynomial_approximation_preserving_finite_returns
    {I : Type*} [Fintype I] [Nonempty I]
    (A : Set ℂ) (hA : IsCompact A) (hfull : IsConnected Aᶜ)
    (g : ℂ → ℂ) (hg : AnalyticOnNhd ℂ g A)
  ...
  obtain ⟨p,hclose,hmarks⟩ :=
    polynomial_approximation_preserving_marked_local_degrees A hA hfull g hg S hSA
      (fun a ha => (hSr a ha).2) τ hτ
  let f : ℂ → ℂ := p.eval
  have hf : AnalyticOnNhd ℂ f univ := AnalyticOnNhd.eval_polynomial p
  have herr : AnalyticOnNhd ℂ (fun z => g z - f z) A :=
    hg.sub (hf.mono (subset_univ _))
  have hregular : ∀ z ∈ A, AnalyticAt ℂ g z → AnalyticAt ℂ f z :=
    fun z _ _ => hf z (mem_univ z)
  ...
  refine ⟨p,hf,herr,(fun z hz => (hclose z hz).trans_le hτeps),
