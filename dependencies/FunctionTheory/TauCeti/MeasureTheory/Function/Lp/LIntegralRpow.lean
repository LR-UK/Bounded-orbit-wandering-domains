...
Compatibility note (FunctionTheory, 17 September 2026): the pinned Mathlib
sets `eLpNorm` to infinity for functions that are not almost everywhere strongly
measurable. Accordingly, the first identity and the comparison theorem below
include explicit measurability assumptions. The Fatou and Hölder statements
are unchanged; their proofs supply measurability already implied by their inputs.
...
theorem eLpNorm_le_of_ae_tendsto_ennreal {α : Type*} [MeasurableSpace α] {μ : Measure α}
  ...
  have hg : AEMeasurable g μ := aemeasurable_of_tendsto_metrizable_ae atTop hf hlim
  have key : eLpNorm g p μ ^ p.toReal ≤ c ^ p.toReal := by
    rw [eLpNorm_rpow_eq_lintegral hp0 hp g μ hg]
    calc ∫⁻ x, g x ^ p.toReal ∂μ
      ...
      _ ≤ c ^ p.toReal := by
          ...
          rw [← eLpNorm_rpow_eq_lintegral hp0 hp (f n) μ (hf n)]
...
theorem eLpNorm_le_eLpNorm_of_lintegral_rpow_le {α : Type*} [MeasurableSpace α] {μ : Measure α}
  ...
  rw [eLpNorm_eq_lintegral_rpow_enorm_toReal hp₀ hp hv,
    eLpNorm_eq_lintegral_rpow_enorm_toReal hp₀ hp hw]
