...
structure ReferenceMapData (D : UniformEscapeData) (n : ℕ) (f : ℂ → ℂ) where
  A : Set ℂ
  U : Set ℂ
  g : ℂ → ℂ
  compact : IsCompact A
  ...
  control : controlDisc n ⊆ interior A
  next : (f^[n]) '' D.K (n + 1) ⊆ interior A
  points : (f^[n]) '' D.P n ⊆ interior A
  old_eq : ∀ z ∈ controlDisc n, g =ᶠ[𝓝 z] f
  translate_eq : ∀ z ∈ (f^[n]) '' D.K (n + 1), g =ᶠ[𝓝 z] (fun w => w + 3)
  trap_eq : ∀ z ∈ (f^[n]) '' D.P n, g =ᶠ[𝓝 z] (fun _ => -3)

theorem exists_reference_map (D : UniformEscapeData) (n : ℕ) (f : ℂ → ℂ)
    (hf : Differentiable ℂ f) (hs : D.StageProperty n f) :
    Nonempty (D.ReferenceMapData n f) := by
  ...
  have hMD : M ⊆ targetDisc n := by
    rintro z ⟨w, hw, rfl⟩
    exact hs.2.1 n le_rfl (hLKn hw)
  have hQD : Q ⊆ targetDisc n := by
    rintro z ⟨w, hw, rfl⟩
    exact hs.2.1 n le_rfl (D.points_subset n hw)
  have hsep : ∀ z ∈ controlDisc n, z.re < (3 * n - 3 / 2 : ℝ) := by
    intro z hz
    have H := controlDisc_re_bound hz
    linarith
  have hsep' : ∀ z ∈ targetDisc n, (3 * n - 3 / 2 : ℝ) < z.re := by
    intro z hz
    have H := (targetDisc_re_bounds hz).1
    linarith
  have hCM : Disjoint (controlDisc n) M := Set.disjoint_left.mpr (fun z hz hzm =>
    (not_lt_of_ge (hsep z hz).le) (hsep' z (hMD hzm)))
  have hCQ : Disjoint (controlDisc n) Q := Set.disjoint_left.mpr (fun z hz hzq =>
  ...
  have hCMfull : IsConnected (controlDisc n ∪ M)ᶜ :=
    isConnected_compl_union_separated _ _ (isCompact_closedBall _ _) hMc
      (isConnected_compl_closedBall _ _) hMf (3 * n - 3 / 2) hsep (fun z hz => hsep' z (hMD hz))
  have hCMQ : Disjoint (controlDisc n ∪ M) Q := disjoint_union_left.mpr ⟨hCQ, hMQ⟩
  have hcompact : IsCompact ((controlDisc n ∪ M) ∪ Q) :=
    ((isCompact_closedBall _ _).union hMc).union hQc
  have hfull : IsConnected ((controlDisc n ∪ M) ∪ Q)ᶜ :=
  ...
  obtain ⟨W₁, g₁, hW₁, hCW₁, hg₁, hold₁, htranslate₁⟩ :=
    exists_holomorphic_gluing (controlDisc n) M univ univ (isCompact_closedBall _ _) hMc hCM
      isOpen_univ isOpen_univ (subset_univ _) (subset_univ _) f (fun z => z + 3)
      hf.differentiableOn (differentiable_id.add_const 3).differentiableOn
  obtain ⟨W, g, hW, hCW, hg, hpiece, htrap⟩ :=
    exists_holomorphic_gluing (controlDisc n ∪ M) Q W₁ univ
