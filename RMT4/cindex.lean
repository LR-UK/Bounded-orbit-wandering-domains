...
lemma circle_integral_eq_zero (hU : IsOpen U) (hr : 0 < r) (hcr : closedBall c r ⊆ U)
  ...
  circleIntegral_eq_zero_of_differentiable_on_off_countable hr.le Set.countable_empty
    ...
    (λ _ hz => f_hol.differentiableAt (hU.mem_nhds (hcr (ball_subset_closedBall (Set.sdiff_subset hz)))))
...
lemma HasFPowerSeriesAt.dslope_order_eventually_ne_zero (hp : HasFPowerSeriesAt f p z₀) (h : p ≠ 0) :
  ...
  have hh : DifferentiableOn ℂ (iterate (swap dslope z₀) p.order f) (Metric.eball z₀ r) :=
    DifferentiableOn.iterate_dslope hf.differentiableOn Metric.isOpen_eball (Metric.mem_eball_self hr)
...
lemma eventually_deriv_div_self_eq (hp : HasFPowerSeriesAt f p z₀) (h : p ≠ 0) :
  ...
  have lh3 : ∀ᶠ z in 𝓝 z₀, ∀ᶠ y in 𝓝 z, f y = (y - z₀) ^ p.order • g y :=
    eventually_eventually_nhds.mpr (.of_forall hp.eq_pow_order_mul_iterate_dslope)
...
lemma cindex_eq_order_aux (hU : IsOpen U) (hr : 0 < r) (h0 : closedBall z₀ r ⊆ U)
  ...
  have e7 : (∮ z in C(z₀, r), c / (z - z₀)) = 2 * π * I * c := by
    have h := circle_integral_sub_center_inv_smul (v := c) (c := z₀) hr
    simp only [smul_eq_mul] at h
    simpa [div_eq_mul_inv, mul_comm _ _⁻¹] using h
