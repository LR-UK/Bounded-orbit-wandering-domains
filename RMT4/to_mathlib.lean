...
lemma has_deriv_at_integral_of_continuous_of_lip
  ...
  exact (hasDerivAt_integral_of_dominated_loc_of_lip (μ := μ) (F := φ) (F' := ψ)
    (bound := fun _ : ℝ => C) (𝕜 := ℂ) (Metric.ball_mem_nhds z₀ δ_pos) h1 h2 h3 h4 h5 h6).2
...
lemma uIoo_eq_uIoc_sdiff_ends : uIoo a b = Ι a b \ {a, b} := by
  ...
  · simp_rw [uIoc, mem_sdiff, mem_Ioc, mem_insert_iff, mem_singleton_iff] at hh
    push Not at hh
...
lemma eventually_mem_uIoo_of_mem_uIoc : ∀ᵐ x, x ∈ Ι a b → x ∈ uIoo a b := by
  ...
  · simpa only [mem_ae_iff, compl_compl, singleton_union]
      using measure_union_null volume_singleton volume_singleton
...
-- `intervalIntegral.integral_congr_uIoo` is now in Mathlib (same statement); the local copy was
-- removed to avoid the name clash.
...
theorem integral_eq_sub' (h : ContDiffOn ℝ 1 f (Icc a b)) (hab : a < b) :
  ...
  · intro t ht
    apply ((h.differentiableOn one_ne_zero) t (Ioo_subset_Icc_self ht)).hasDerivWithinAt.hasDerivAt
...
theorem integral_derivWithin_smul_comp
  ...
  apply (hg.differentiableOn one_ne_zero t (Set.uIoo_subset_uIcc_self ht)).hasDerivWithinAt.mono_of_mem_nhdsWithin
