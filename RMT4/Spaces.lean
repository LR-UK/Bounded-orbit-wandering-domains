...
lemma isClosed_𝓗 (hU : IsOpen U) : IsClosed (𝓗 U) := by
  ...
  · exact (tendsto_𝓒_iff hU).mp (tendsto_id.mono_left inf_le_left)
  · exact Filter.mem_inf_of_right (Filter.mem_principal.mpr (fun g hg => hg))
...
lemma 𝓑_const {Q : Set ℂ} : 𝓑 U (fun _ => Q) = {f ∈ 𝓗 U | MapsTo f U Q} := by
  ext f
  simp only [𝓑, mem_setOf_eq, and_congr_right_iff]
  refine fun _ => ⟨fun h => ?_, fun h K hK => h.mono_left hK.1⟩
  simpa using mapsTo_sUnion.2 h
