...
lemma non_injective_schwarz {f : ℂ → ℂ} (f_diff : DifferentiableOn ℂ f 𝔻)
  ...
  case pos =>
    have g_lin : EqOn g (λ (z : ℂ) => z • deriv g 0) (ball 0 1) := by
      have h2 : MapsTo g (ball 0 1) (closedBall (g 0) 1) :=
        (by rwa [g_0_eq_0] : MapsTo g (ball 0 1) (ball (g 0) 1)).mono_right Metric.ball_subset_closedBall
  ...
  case neg =>
    have g_maps_cb : MapsTo g (ball 0 1) (closedBall (g 0) 1) := by
      rw [g_0_eq_0]; exact g_maps.mono_right Metric.ball_subset_closedBall
    have g'0_le_1 := norm_deriv_le_one_of_mapsTo_ball g_diff g_maps_cb zero_lt_one
