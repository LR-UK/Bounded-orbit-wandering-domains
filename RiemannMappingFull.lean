...
Adapted from Yury Kudryashov, mathlib4 PR 33505, commit
`d43061d911b1aeae0788591da437a3b115098962`.
Upstream Apache 2.0 licence retained. Local adaptations target Mathlib 4.34.0.
-/
...
theorem exists_branch_log {X : Type*} [TopologicalSpace X] [LocallyPathConnectedSpace X] {U : Set X}
  ...
  case hg_inv =>
    ...
    simpa [hg, Set.domRestrict, ContinuousMap.coe_mk] using congr($hf x)
...
theorem exist_map_unitDisc_injOn_norm_deriv_gt {U : Set ℂ} (hUo : IsOpen U)
  ...
  have hdg : ∀ z ∈ U, HasDerivAt (g · : ℂ → ℂ)
    ...
    all_goals simp [Function.comp_def, field, hg₀ z hz] <;> ring
