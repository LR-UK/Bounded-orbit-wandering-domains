...
def pathCoverSheet (pc : PathCover x₀) (U : Set M) : Set (PathCover x₀) :=
  {qc | ∃ η : Path pc.pt qc.pt, (∀ t, η t ∈ U) ∧
    qc.cls = pc.cls.trans (Path.Homotopic.Quotient.mk η)}
...
path. -/
def pathCoverBase : PathCover x₀ := ⟨x₀, Path.Homotopic.Quotient.mk (Path.refl x₀)⟩
...
theorem pathCoverProj_isCoveringMap : IsCoveringMap (pathCoverProj x₀) := by
  ...
  have hq1 : ∀ {a b c : M} (p : Path a b) (q : Path b c),
      Path.Homotopic.Quotient.trans (Path.Homotopic.Quotient.mk p)
        (Path.Homotopic.Quotient.mk q) = Path.Homotopic.Quotient.mk (p.trans q) := by
    intros
    rfl
  ...
  have hslice : ∀ (rc : PathCover x₀) (V : Set M), IsOpen V → ∀ hrcV : rc.pt ∈ V,
      ...
      Continuous fun y : ↥V =>
        (⟨y.val, rc.cls.trans (Path.Homotopic.Quotient.mk (lin rc.pt hrcV y.val y.2))⟩ :
          PathCover x₀) := by
    ...
    intro y hy
    obtain ⟨η₀, hη₀, hcls₀⟩ := hy
    have hcls₀' : rc.cls.trans (Path.Homotopic.Quotient.mk (lin rc.pt hrcV y.val y.2)) =
    ...
    have hstep : Path.Homotopic.Quotient.mk (lin rc.pt hrcV w.val w.2) =
        Path.Homotopic.Quotient.mk ((lin rc.pt hrcV y.val y.2).trans (lin' y.val hyV' w.val hw)) :=
      hhomo _ _ _ _ (fun t => hlin _ _ _ _ t)
        (htmem V _ _ (fun t => hlin _ _ _ _ t) fun t => (hV'sub (hlin' _ _ _ _ t)).2)
    have hfin : rc.cls.trans (Path.Homotopic.Quotient.mk (lin rc.pt hrcV w.val w.2)) =
        pc.cls.trans (Path.Homotopic.Quotient.mk (η₀.trans (lin' y.val hyV' w.val hw))) := by
  ...
  have hret : ∀ (qc : PathCover x₀) (hq : qc.pt ∈ V) (rc : PathCover x₀),
      ...
      rc.cls.trans (Path.Homotopic.Quotient.mk (lin rc.pt hr x hxV)) =
        qc.cls.trans (Path.Homotopic.Quotient.mk (lin qc.pt hq x hxV)) := by
    intro qc hq rc hrc hr
    obtain ⟨η, hη, hcls⟩ := hrc
    have hcls' : rc.cls = qc.cls.trans (Path.Homotopic.Quotient.mk η) := hcls
  ...
  refine ⟨{ toFun := fun qc =>
              (⟨pathCoverProj x₀ qc.val, qc.2⟩,
                qc.val.cls.trans (Path.Homotopic.Quotient.mk (lin qc.val.pt qc.2 x hxV)))
            invFun := fun p =>
              ⟨⟨p.1.val, p.2.trans (Path.Homotopic.Quotient.mk (lin x hxV p.1.val p.1.2))⟩, p.1.2⟩
            left_inv := ?_
            right_inv := ?_
            continuous_toFun := ?_
            continuous_invFun := ?_ }, fun _ => rfl⟩
  · rintro ⟨⟨pt, cls⟩, hq⟩
    have hloop : Path.Homotopic.Quotient.trans (Path.Homotopic.Quotient.mk (lin pt hq x hxV))
        (Path.Homotopic.Quotient.mk (lin x hxV pt hq)) = Path.Homotopic.Quotient.refl pt := by
      rw [hq1]
      exact hhomo _ _ _ _
        (htmem V (lin pt hq x hxV) (lin x hxV pt hq) (fun t => hlin _ _ _ _ t)
          fun t => hlin _ _ _ _ t)
        fun _ => hq
    have hC : (cls.trans (Path.Homotopic.Quotient.mk (lin pt hq x hxV))).trans
        (Path.Homotopic.Quotient.mk (lin x hxV pt hq)) = cls := by
  ...
  · rintro ⟨⟨y, hy⟩, c⟩
    have hloop : Path.Homotopic.Quotient.trans (Path.Homotopic.Quotient.mk (lin x hxV y hy))
        (Path.Homotopic.Quotient.mk (lin y hy x hxV)) = Path.Homotopic.Quotient.refl x := by
      rw [hq1]
      exact hhomo _ _ _ _
        (htmem V (lin x hxV y hy) (lin y hy x hxV) (fun t => hlin _ _ _ _ t)
          fun t => hlin _ _ _ _ t)
        fun _ => hxV
    have hC : (c.trans (Path.Homotopic.Quotient.mk (lin x hxV y hy))).trans
        (Path.Homotopic.Quotient.mk (lin y hy x hxV)) = c := by
      rw [Path.Homotopic.Quotient.trans_assoc, hloop, Path.Homotopic.Quotient.trans_refl]
  ...
  · refine Continuous.prodMk
    ...
    have h1 : rc.val.cls.trans (Path.Homotopic.Quotient.mk (lin rc.val.pt rc.2 x hxV)) =
        qc.val.cls.trans (Path.Homotopic.Quotient.mk (lin qc.val.pt qc.2 x hxV)) :=
      hret qc.val qc.2 rc.val hrc rc.2
    have h2 : qc.val.cls.trans (Path.Homotopic.Quotient.mk (lin qc.val.pt qc.2 x hxV)) = b := hqc
    exact h1.trans h2
...
theorem exists_pathCover_openPartialHomeomorph (pc : PathCover x₀) :
  ...
  refine ⟨{ toFun := pathCoverProj x₀
            invFun := fun y =>
              if h : y ∈ V then
                ⟨y, pc.cls.trans (Path.Homotopic.Quotient.mk (lin pc.pt hpcV y h))⟩
              else pc
  ...
  · intro qc hqc
    ...
    have hcls' : qc.cls = pc.cls.trans (Path.Homotopic.Quotient.mk η) := hcls
    have hq : pathCoverProj x₀ qc ∈ V := by
      have h := hη 1
      rw [η.target] at h
      exact h
    have hfin : pc.cls.trans
        (Path.Homotopic.Quotient.mk (lin pc.pt hpcV (pathCoverProj x₀ qc) hq)) = qc.cls := by
      rw [hhomo _ _ (lin pc.pt hpcV (pathCoverProj x₀ qc) hq) η
        (fun t => hlin _ _ _ _ t) hη]
      exact hcls'.symm
    change (if h : pathCoverProj x₀ qc ∈ V then
        (⟨pathCoverProj x₀ qc,
          pc.cls.trans (Path.Homotopic.Quotient.mk (lin pc.pt hpcV (pathCoverProj x₀ qc) h))⟩ :
            PathCover x₀)
      else pc) = qc
    rw [dite_eq_left hq]
  ...
  · rw [continuousOn_iff_continuous_domRestrict]
    ...
    change (⟨y.val, pc.cls.trans (Path.Homotopic.Quotient.mk (lin pc.pt hpcV y.val y.2))⟩ :
        PathCover x₀) =
      if h : (y : M) ∈ V then
        ⟨(y : M), pc.cls.trans (Path.Homotopic.Quotient.mk (lin pc.pt hpcV (y : M) h))⟩
      else pc
    rw [dite_eq_left y.2]
...
theorem pathConnectedSpace_pathCover [ConnectedSpace M] :
  ...
  have hjoin : ∀ pc : PathCover x₀, Joined (pathCoverBase x₀) pc := by
    ...
    have hLcont : Continuous fun t : I =>
    ...
    have hL1 : (⟨γ 1, Path.Homotopic.Quotient.mk (sp 1)⟩ : PathCover x₀) =
        ⟨y, Path.Homotopic.Quotient.mk γ⟩ := by
...
theorem simplyConnectedSpace_pathCover [ConnectedSpace M] :
  ...
  have hmono : ∀ {a b : M} (γ : Path a b) (Λ : I → PathCover x₀), Continuous Λ →
      ...
      Λ 0 = ⟨a, d⟩ → Λ 1 = ⟨b, d.trans (Path.Homotopic.Quotient.mk γ)⟩ := by
    ...
    -- Continuity of the canonical lift into the sheet topology.
    have hLcont : Continuous fun t : I =>
        (⟨γ t, d.trans (Path.Homotopic.Quotient.mk (sp t))⟩ : PathCover x₀) := by
      refine continuous_generateFrom_iff.mpr ?_
      rintro s ⟨rc, U, hUopen, -, rfl⟩
      rw [isOpen_iff_forall_mem_open]
      intro t₀ ht₀
      obtain ⟨η₀, hη₀, hcls₀⟩ := ht₀
      have hcls₀' : d.trans (Path.Homotopic.Quotient.mk (sp t₀)) =
          rc.cls.trans (Path.Homotopic.Quotient.mk η₀) := hcls₀
      have hγt₀U : γ t₀ ∈ U := by
        have h := hη₀ 1
        rw [η₀.target] at h
        exact h
      ...
      have hclseq : Path.Homotopic.Quotient.mk (sp t) =
        ...
        · intro u
          ...
          · rw [hcfun]
      refine ⟨η₀.trans c, htmem, ?_⟩
      have hfin : d.trans (Path.Homotopic.Quotient.mk (sp t)) =
          rc.cls.trans (Path.Homotopic.Quotient.mk (η₀.trans c)) := by
    ...
    have hLeq : Λ = fun t : I =>
      ...
      · rw [hΛ0]
        have hsp0 : sp 0 = (Path.refl a).cast rfl γ.source := by
          ...
          exact γ.extend_zero
        rw [hsp0]
        have h1 : Path.Homotopic.Quotient.mk ((Path.refl a).cast rfl γ.source) =
    ...
    have hL1 : Λ 1 = ⟨γ 1, d.trans (Path.Homotopic.Quotient.mk (sp 1))⟩ := congrFun hLeq 1
    have hsp1 : sp 1 = γ.cast rfl γ.target := by
      ...
      exact γ.extend_extends' u
    rw [hL1, hsp1]
    have h2 : Path.Homotopic.Quotient.mk (γ.cast rfl γ.target) =
        Path.Homotopic.Quotient.cast (Path.Homotopic.Quotient.mk γ) rfl γ.target := rfl
    rw [h2, hqct γ.target d (Path.Homotopic.Quotient.mk γ)]
    exact (hcast γ.target (d.trans (Path.Homotopic.Quotient.mk γ))).symm
  ...
  have h₂ := hmono (P₂.map hcont) ⇑P₂ P₂.continuous (fun t => rfl) pc.cls P₂.source
  have e₁ : qc = ⟨pathCoverProj x₀ qc, pc.cls.trans (Path.Homotopic.Quotient.mk (P₁.map hcont))⟩ :=
    P₁.target.symm.trans h₁
  have e₂ : qc = ⟨pathCoverProj x₀ qc, pc.cls.trans (Path.Homotopic.Quotient.mk (P₂.map hcont))⟩ :=
    P₂.target.symm.trans h₂
  have e₃ := e₁.symm.trans e₂
  have e₄ : pc.cls.trans (Path.Homotopic.Quotient.mk (P₁.map hcont)) =
      pc.cls.trans (Path.Homotopic.Quotient.mk (P₂.map hcont)) := by
    injection e₃
...
theorem exists_pathCoverDeck_diffeomorph (γ : Path.Homotopic.Quotient x₀ x₀) :
  ...
  have hcont : ∀ δ : Path.Homotopic.Quotient x₀ x₀, Continuous (pathCoverDeck x₀ δ) := by
    ...
    have hpre : pathCoverDeck x₀ δ ⁻¹' pathCoverSheet x₀ pc U =
      ...
      constructor
      · intro h
        -- Read the membership at the endpoint `qc.pt` of `qc` itself, so that the endpoints of
        -- `η` are syntactically those of `qc.cls`.
        obtain ⟨η, hη, hcls⟩ : ∃ η : Path pc.pt qc.pt, (∀ t, η t ∈ U) ∧
            Path.Homotopic.Quotient.trans δ qc.cls =
              Path.Homotopic.Quotient.trans pc.cls (Path.Homotopic.Quotient.mk η) := h
        refine ⟨η, hη, ?_⟩
